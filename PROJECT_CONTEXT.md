...
### رفعِ به‌هم‌ریختنِ bidi (RTL/LTR) در متنِ ترجمه‌شده — انجام شد
- علت: خطِ فارسی که عدد/کلمه‌ی انگلیسی/@یوزرنیم/لینک دارد، جهتِ پایه‌اش LTR تشخیص داده می‌شد.
- فیکس: `TranslateAlert2.fixBidi(text, toLng)` — اگر زبانِ مقصد RTL باشد (fa/ar/he/ur/…):
  هر خط یک `‏` (RLM) در ابتدا می‌گیرد (جهتِ پایه RTL) و هر run از کاراکترهای ASCIIِ حاوی
  حرف/عدد در ایزوله‌ی `⁦…⁩` (LRI…PDI) پیچیده می‌شود. زبانِ LTR دست‌نخورده.
- در **هر دو** خروجی صدا زده می‌شود: `aiTranslate` (بعد از trim) و مسیرِ گوگل (قبل از finalResult).
- اسکریپت مستقل: `fix_bidi.py/.bat` (بکاپ `.bak13`)؛ apply_mods بخش ۴.۶ (کلید `fix_bidi`).
